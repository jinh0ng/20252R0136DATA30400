...
EMB_SUBDIR="${EMB_SUBDIR:-embeddings_bge}"
PID_SUBDIR="${PID_SUBDIR:-embeddings_bge}"
DATA_DIR="${DATA_DIR:-$REPO_DIR/data_bge}"
OUTPUTS_DIR="${OUTPUTS_DIR:-$REPO_DIR/outputs_bge}"
...
OUT_NAME="${OUT_NAME:-2021350218_final.csv}"
...
echo_section "1) MAKE EMBEDDINGS (BGE) - SKIP IF EXISTS"
if [[ "$FORCE_EMB" == "1" ]]; then
  FORCE_FLAG="--force"
else
  FORCE_FLAG=""
